//***************************************************************
//       EmbeddedWbDEMO ver D2005 (oct. 30 , 2005)              *
//                                                              *
//                     For Delphi  2005                         *
//                     Freeware Demo                            *
//                        by  bsalsa                            *
//                                                              *
// Thanks to tom@swissdelphicenter.ch for the demo contribution *
//                                                              *
//***************************************************************
...
type
  TfrmMain = class(TForm)
    ...
    DisableRightClickMenu: TMenuItem;
    MMEdit: TMenuItem;
    edtSearch: TEdit;
    ...
    SearchandHighlightall: TMenuItem;
    ...
    ViewPageFieldsAsAList1: TMenuItem;
    ViewPageImagesAsAList1: TMenuItem;
    ViewPageLinksAsAList1: TMenuItem;
    ViewPagePropertiesAsAList1: TMenuItem;
    ...
    procedure DisableRightClickMenuClick(Sender: TObject);
    ...
    procedure EmbeddedWB1Refresh(Sender: TObject; CmdID: Integer; var Cancel: Boolean);
    procedure EmbeddedWB1ScriptError(Sender: TObject; ErrorLine, ErrorCharacter,
     ErrorCode, ErrorMessage, ErrorUrl: string; var ContinueScript, Showdialog: Boolean);
    ...
    procedure SearchandHighlightallClick(Sender: TObject);
...
begin
   ...
   UpdateControls;
  if FileExists(ExtractFilePath(ParamStr(0)) + 'Site.html') then DeleteFile('Site.html');
...
procedure TfrmMain.SearchandHighlightallClick(Sender: TObject);
begin
  // EmbeddedWB1.SearchAndHighlight('Find', true );
end;
...
procedure TfrmMain.DisableRightClickMenuClick(Sender: TObject);
begin
  if DisableRightClickMenu.Checked then
   EmbeddedWB1.RightClickMenu.DisableAllMenus:=true
   else EmbeddedWB1.RightClickMenu.DisableAllMenus:=false;
end;
...
ZoomIndex : integer;
begin
   ZoomIndex := EmbeddedWB1.ZoomValue;
    case ZoomIndex of
...
begin
 if EmbeddedWB1.SetCharartersSet(TMenuItem(Sender).Hint) then TMenuItem(Sender).Checked := True;
end;
...
var
 st : string;
begin
     st:= EmbeddedWB1.GetCookie;   SetLength(st, 180);
...
var
 st : string;
begin
  st := EmbeddedWB1.GetCookiesPath ; ShowMessage(st);
end;
...
var
 st : string;
begin
  st := EmbeddedWB1.GetHistoryPath ; ShowMessage(st);
end;
...
begin
   ...
   If tbBlock.Down then Cancel:= True else EmbeddedWB1.Navigate(InitialURL);
end;
...
begin
   ...
   If tbBlock.Marked then Cancel:= True else Cancel := false;
end;
...
procedure TfrmMain.EmbeddedWB1Refresh(Sender: TObject; CmdID: Integer;
  var Cancel: Boolean);
begin
...
procedure TfrmMain.EmbeddedWB1FullScreen(ASender: TObject;
  FullScreen: WordBool);
begin
  If FullScreen then AddEventLog('OnFullScreen: '+ 'FullScreen');
end;

procedure TfrmMain.EmbeddedWB1Visible(ASender: TObject; Visible: WordBool);
begin
  if Visible then AddEventLog('OnVisible: '+ 'Visible');
end;
...
begin
   ...
   If ExportFavorite1.SuccessFlag then EmbeddedWB1.Navigate(ExportFavorite1.NavigatePath);
end;
...
begin
   ...
   UpdateView();
  If ImportFavorite1.SuccessFlag then EmbeddedWB1.ShowOrganizeFavorites;
end;
...
begin
  If EmbeddedWB1.CheckIfInRestricredList(EmbeddedWB1.LocationURL)then
...
begin
  if EmbeddedWB1.CheckIfInTrustedList(EmbeddedWB1.LocationURL) then
...
else ShowMessage('The site is not secured.');
end;
...
begin
 with aNode do
  begin
       if Text = 'Tools'              then ImageIndex := 0
  else if Text = 'Organize favorites' then ImageIndex := 1
  else if Text = 'Add To favorites'   then ImageIndex := 2
  else if Text = 'Import favorites'   then ImageIndex := 3
  else if Text = 'Export favorites'   then ImageIndex := 4
  else if Text = 'Favorites'          then ImageIndex := 5
  else if (pos ( 'Links', Text ) > 0 ) or (pos('Imported', Text) > 0 )
  then
    begin
       HasChildren:=true;
       ImageIndex :=6
