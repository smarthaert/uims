...
//--------------------------------------------------------------------------
  ...
  // 用同样的颜色调整矩阵调整所有颜色值（包括灰色底纹）, 不调整灰色底纹, 只调整灰色底纹
...
*******************************************************************************)
  ...
  TLineJoin = (ljMiter, ljBevel, LineJoinRound, ljMiterClipped);
...
//--------------------------------------------------------------------------
  ...
  TStringFormatFlag = (sfDirectionRightToLeft, sfDirectionVertical, sfNoFitBlackBox,
    sfDisplayFormatControl = 5, sfsNoFontFallback = 10, sfMeasureTrailingSpaces,
...
//--------------------------------------------------------------------------
  ...
  private
    ...
    function GetNearestColor: TARGB;
  ...
  public
    ...
    property NearestColor: TARGB read GetNearestColor;
  end;
...
function TGpImage.GetFlags: Integer;
begin
  CheckStatus(GdipGetImageFlags(Native, Result));
end;
...
function TGpGraphics.GetNearestColor: TARGB;
begin
...
asm
  push  eax
  push  eax
  and   eax, 0ff0000h // Red
  shr   eax, 10h
  pop   edx
  and   edx, 0ff00h   // Green
  or    eax, edx
  pop   edx
  and   edx, 0ffh     // Blue
  shl   edx, 10h
...
begin
  Result := ARGBFromCOLORREF(255, Rgb);
end;
...
begin
  dest.X := Max(a.X, b.X);
  dest.Y := Max(a.Y, b.Y);
  dest.Width := Min(a.X + a.Width, b.X + b.Width) - dest.X;
...
begin
  ...
  dest.Height := Max(a.Y + a.Height, b.Y + b.Height) - dest.Y;
  Result := not IsEmptyArea(dest);
end;
