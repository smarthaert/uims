...
begin
try
  if AMax < AMin then exit; //raise EInvalidOperation.CreateFmt(SPropertyOutOfRange, [Self.Classname]);
  if not F32BitMode and ((AMin < 0) or (AMin > Limit16) or (AMax < 0) or
    (AMax > Limit16)) then exit; //ProgressLimitError;
...
begin
try
  if not F32BitMode and ((Value < 0) or (Value > Limit16)) then exit;  //ProgressLimitError;
    ...
    case Message.Msg of
      ...
      WM_LBUTTONDOWN, WM_LBUTTONDBLCLK:
        ...
        begin
          ...
          if (TToolButton(Control).Style <> tbsDropDown) and   //kkk
...
try
  ...
  if Assigned(FOnAdvancedCustomDrawButton) then FOnAdvancedCustomDrawButton(Self, button, State, Stage, Flags, Result);  //kk
except end;   //kkkk
end;
