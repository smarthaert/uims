...
uses
  Windows, SysUtils, TypInfo, var_, const_;

const
  OutputFile = 'CheckMemory.Log';
  MaxCount = High(Word);
...
finalization
try
  OutputFileFullName := {ExtractFileDir(ParamStr(0)) + '\' +} MyDir + DataDir + '\' + OutputFile;
  DeleteFile(OutputFileFullName);
  SetMemoryManager(OldMemMgr);
  if (GetMemCount - FreeMemCount) <> 0 then
    SnapCurrMemStatToFile(OutputFileFullName);
except end;
