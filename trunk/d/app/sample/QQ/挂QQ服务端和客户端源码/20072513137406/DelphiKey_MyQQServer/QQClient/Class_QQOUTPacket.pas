...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls,Class_QQTEA,Class_Record;
Type
   TQQOutPacket=Class
   private
   public
      ...
      function ChangeStatus(QQNum: integer; BOnLine, BFakeCam: Boolean;
      SessionKey: array of Byte; var nSendLen: integer): TMYByte;
   end;
...
begin
    ...
    Plain[High(Plain) - 12] := 0 ;
    Plain[High(Plain) - 11] := 9 ;
    Plain[High(Plain) - 10]:=255 ;
    Plain[High(Plain) - 9] := 0;
    Plain[High(Plain) - 8] := 0;
    Plain[High(Plain) - 7] := 0  ;
    Plain[High(Plain) - 6] := 134;
    Plain[High(Plain) - 5] := 0  ;
    Plain[High(Plain) - 4] := $CB ;
...
function TQQOutPacket.ChangeStatus(QQNum: integer; BOnLine,
  BFakeCam: Boolean; SessionKey: array of Byte;
  var nSendLen: integer): TMYByte;
var
   QQBuff:array of byte;
   i:integer;
   Crypt,Packet:TMYByte;
...
begin
    ...
    Packet[10]:= QQBuff[0]    ;//
    CopyMemory(@Packet[11], @Crypt[0],nSendLen);
    if BOnLine  then Packet[11]:= 1 else Packet[11]:= 0;
    if BFakeCam then Packet[12]:= 1 else Packet[12]:= 0;
    Packet[13]:=0;
    Packet[14]:=0;
    Packet[15]:=0;
    Packet[nSendLen+15] := 3;
    Copymemory(@Result[0], @Packet[0],nSendLen+17);
    nSendLen :=nSendLen+17;
end;
