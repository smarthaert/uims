...
begin
  ...
  begin
    ...
    begin
      ...
      s_caption_9 := '空  ' + Trim(s_tmp);
      s_caption_11 := IntToStr(i_short_mark);

        //MAINFORM.Label9.Caption :='空  '+Trim(s_tmp);
    ...
    begin
      ...
      s_caption_8 := '多  ' + Trim(s_tmp);
      s_caption_10 := IntToStr(i_got_it_mark);
        //MAINFORM.Label8.Caption :='多  '+Trim(s_tmp);
...
begin
 ...
 //cout<<as_k_chg_ma_rsi[i_s_k_chg_index_wk].ai_ma[2]<<"\t"<<as_k_chg_ma_rsi[i_s_k_chg_index_wk].ai_rsi[0]<<"\t"<<i_s_k_chg_index_wk<<"\t"<<as_k_chg[i_s_k_chg_index_wk].i_date<<endl;
  ...
  //MAINFORM.Label1.Caption:='卖  '+FloatToStrF(lpData.sale_low_price,ffFixed,7,2);
  //MAINFORM.Label2.Caption:=IntToStr(lpData.sale_low_amount);

  //MAINFORM.Label5.Caption:='买  '+FloatToStrF(lpData.buy_high_price,ffFixed,7,2);
  //MAINFORM.Label6.Caption:=IntToStr(lpData.buy_high_amount);

  //MAINFORM.Label7.Caption:=pc_username;

  s_caption_1 := '卖  ' + FloatToStrF(lpData.sale_low_price, ffFixed, 7, 2);
  s_caption_2 := IntToStr(lpData.sale_low_amount);

  s_caption_5 := '买  ' + FloatToStrF(lpData.buy_high_price, ffFixed, 7, 2);
...
begin
  ...
  if MAINFORM.N4.Checked = True then s_caption_15 := '锁' else s_caption_15 := ' ';
  if MAINFORM.N5.Checked = True then s_caption_16 := '锁' else s_caption_16 := ' ';
  ...
  MAINFORM.Label_up_power.Caption := '多方量能 ' + IntToStr(i_up_count_ex) + ' : ' + IntToStr(i_up_vol_sum);
  MAINFORM.Label_down_power.Caption := '空方量能 ' + IntToStr(i_down_count_ex) + ' : ' + IntToStr(i_down_vol_sum);
...
begin
 ...
 //业务操作委托下单
  lpReqMsg := NewFuMessage(MSG_TYPE_GET_PROFIT, Integer(MSG_MODE_REQUEST)); //委托下单
  lpAnsMsg := NewFuMessage(MSG_TYPE_UNKNOWN, Integer(MSG_MODE_ANSWER)); //接收消息(无关消息类型)
...
begin
  ...
  if i_only_1 = 1 then s_caption_14 := '单发' else s_caption_14 := '群发';
  ...
  begin
    ...
    begin
      s_caption_status := '已锁多仓，不得进行冲动的平多操作！';
    end
    else
    begin
    //for loop add
