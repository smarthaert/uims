...
public class BasicHttpClient {
	...
	protected static String postText(String url, Map<String, String> values) {
		...
		synchronized (httpclient) {
			try {
				...
				responseBody = httpclient.execute(httppost, brh, localContext);
				cookieDisplay(cookieStroe);
			} catch (Exception e) {
				e.printStackTrace();
				responseBody = null;
	 ...
	 * @return
	 */
	protected static Document postTextToDoc(String url,
		...
		synchronized (httpclient) {
			try {
				httppost.setEntity(new UrlEncodedFormEntity(nvps, HTTP.UTF_8));
				page = httpclient.execute(httppost, jrh, localContext);
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				httppost.abort();
	...
	protected static JSONObject getTextToJson(String url) {
		...
		synchronized (httpclient) {
			try {
				json = httpclient.execute(httpget, jrh, localContext);
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				httpget.abort();
	 ...
	 * @return
	 */
	protected static JSONObject postTextToJson(String url,
			Map<String, String> values) {
		HttpPost httppost = new HttpPost(url);
		List<NameValuePair> nvps = new ArrayList<NameValuePair>();
		for (Map.Entry<String, String> e : values.entrySet()) {
		...
		synchronized (httpclient) {
			try {
				httppost.setEntity(new UrlEncodedFormEntity(nvps, HTTP.UTF_8));
				json = httpclient.execute(httppost, jrh, localContext);
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				httppost.abort();
